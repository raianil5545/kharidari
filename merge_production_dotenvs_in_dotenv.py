...
def test_merge(tmpdir_factory, merged_file_count: int, append_linesep: bool):
    temp_dir_path = Path(str(tmpdir_factory.getbasetemp()))
    output_file_path = temp_dir_path / '.env'
    expected_output_file_content = ""
    merged_file_paths = []
    for i in range(merged_file_count):
        merged_file_ord = i + 1
        merged_filename = f".service{merged_file_ord}"
        merged_file_path = temp_dir_path / merged_filename
        ...
        merged_file_paths.append(merged_file_paths)
...
def main():
    merge(DOTENV_FILE_PATH, PRODUCTION_DOTENVS_FILE_PATHS)
